...
// ---------- 读书分享记录（按用户 + 课程 + 课） ----------
export function getLessonShare(uid, slug, lessonId) {
  if (typeof window === 'undefined') return {};
  if (!uid) return {};
  const key = `share:${uid}:${slug}:${lessonId}`;
  try {
    return JSON.parse(localStorage.getItem(key)) || {};
  } catch { return {}; }
}
export function saveLessonShare(uid, slug, lessonId, data) {
  if (typeof window === 'undefined' || !uid) return;
  try {
    localStorage.setItem(`share:${uid}:${slug}:${lessonId}`, JSON.stringify(data));
    window.dispatchEvent(new CustomEvent('xz-store', { detail: `share:${slug}:${lessonId}` }));
  } catch {}
}
export function countShares(uid) {
  if (typeof window === 'undefined' || !uid) return 0;
  let n = 0;
  try {
    for (let i = 0; i < localStorage.length; i++) {
      const k = localStorage.key(i);
      if (k && k.startsWith('share:' + uid + ':')) n++;
    }
  } catch {}
  return n;
}
